...
huepi.prototype.PortalDiscoverLocalBridges = function()
{
  ...
  self.Username = '';
  self.LocalBridges = [];
  $.ajax({ type: 'GET', url: 'https://discovery.meethue.com/', success: function(data) {
    try {
      if (data.length > 0) {
 ...
 */
huepi.prototype.HelperRGBtoHueAngSatBri = function(Red, Green, Blue)
{
  var Ang, Sat, Bri;
...
/**
 * Note: For bridges < 1.4 lights must be on, otherwise they won't be saved in the group. For 1.4 there is no such restriction.
 * @param {string} Name New name of the light Range [1..32]
